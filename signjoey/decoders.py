...
class TransformerDecoder(Decoder):
    ...
    def __init__(
        ...
        **kwargs
        ...
        :param attention_type: normalization used for both the masked
            self-attention and the encoder-decoder (cross) attention
            scores, "softmax" (default) or "reluformer" (see
            `signjoey.relu_attention.ReLUFormerAttention`)
        :param kwargs:
        """
        ...
        # aggregated ReLUFormer regularization loss from the last
        # forward pass (None for softmax attention / at eval time)
        self.reg_loss = None
    ...
    def forward(
        ...
        **kwargs
        ...
        # collect ReLUFormer regularization losses (if any) from all layers
        reg_losses = [l.reg_loss for l in self.layers if l.reg_loss is not None]
        self.reg_loss = torch.stack(reg_losses).sum() if reg_losses else None
