...
class SignModel(nn.Module):
    ...
    def get_attention_reg_loss(self) -> Tensor:
        """
        Sum of the ReLUFormer attention-regularization losses (Eq. 7 in
        "A Study on ReLU and Softmax in Transformer") collected from the
        encoder and, if present, the decoder during the last forward
        pass. Returns `None` if no sub-module used "reluformer"
        attention, or if the model is in eval mode (the regularizer is
        only meaningful/computed during training).

        :return: scalar regularization loss tensor, or None
        """
        reg_losses = []
    ...
    def get_loss_for_batch(
        ...
        :param attn_reg_loss_weight: Weight for the ReLUFormer attention
            regularization loss (Eq. 7 of the ReLUFormer paper). Has no
            effect unless encoder/decoder use "reluformer" attention
            and the model is in training mode.
        ...
        # Add the ReLUFormer attention regularization loss (if any),
        # scaled by attn_reg_loss_weight, to whichever objective(s)
        # backprop through the encoder/decoder attention modules.
